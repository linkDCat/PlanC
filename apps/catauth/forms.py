...
class login_form(forms.Form,FormMixin):
    telephone=forms.CharField(max_length=11)
    password=forms.CharField(max_length=20,min_length=6,error_messages={"max_length":'密码最多不能超过20个字符',"min_length":"密码最多不能少于6个字符"})
    remember =forms.IntegerField(required=False)

class register_form(forms.Form,FormMixin):
    telephone = forms.CharField(max_length=11)
    username = forms.CharField(max_length=20,min_length=4)
    password1 = forms.CharField(max_length=20, min_length=6,
                               error_messages={"max_length": '密码最多不能超过20个字符', "min_length": "密码最多不能少于6个字符"})
    ...
    def clean(self):
        ...
        exists = Front_User.objects.filter(telephone=telephone).exists()
